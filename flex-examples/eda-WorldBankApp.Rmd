...
sliderInput("year", "Year", min = min(years), max = max(years), 
            value = min(years), step = 1, sep = "", animate = TRUE)

selectizeInput("country", "Select a country", choices = unique(WorldBank$country),
    selected = c("United States", "Thailand"), multiple = TRUE)

selectizeInput("region", "Select a region", choices = unique(WorldBank$region),
...
```{r}
# plot1: life expectancy by year
WorldBank1 <- reactive({
    if (!is.null(input$region)) {
        WorldBank <- WorldBank[input$region %in% input$region, ]
    }
    WorldBank
})
...
reactive({
    WorldBank1() %>%
        ggvis(x = ~year) %>%
        group_by(country) %>%
        layer_lines(x = ~year, y = ~life.expectancy, stroke = ~region, opacity := 0.5, 
                    opacity.hover := 1, strokeWidth := 3) %>%
        add_data(currentYear) %>%
        layer_rects(x = ~year - 0.5, x2 = ~year + 0.5, y = rng[1], y2 = rng[2]) %>%
...
```{r}
# plot2: life expectancy vs fertility rate
